...
# Modified by nullata 2026
#
# Nullata MySQL library

# shellcheck disable=SC1090,SC1091,SC2119,SC2120

# Load Generic Libraries
. /opt/nullata/scripts/libfile.sh
. /opt/nullata/scripts/liblog.sh
. /opt/nullata/scripts/libfs.sh
. /opt/nullata/scripts/libos.sh
. /opt/nullata/scripts/libservice.sh
. /opt/nullata/scripts/libvalidations.sh
. /opt/nullata/scripts/libversion.sh
...
mariadb_initialize() {
    ...
    if [[ -f "${DB_CONF_DIR}/my_custom.cnf" ]]; then
        if is_file_writable "${DB_CONF_DIR}/nullata/my_custom.cnf"; then
            info "Injecting custom configuration 'my_custom.cnf'"
            cat "${DB_CONF_DIR}/my_custom.cnf" > "${DB_CONF_DIR}/nullata/my_custom.cnf"
            if ! grep --silent "!include ${DB_CONF_DIR}/nullata/my_custom.cnf" "${DB_CONF_FILE}"; then
                echo "!include ${DB_CONF_DIR}/nullata/my_custom.cnf" >> "${DB_CONF_FILE}"
            fi
        else
            warn "Could not inject custom configuration for the ${DB_FLAVOR} configuration file '$DB_CONF_DIR/nullata/my_custom.cnf' because it is not writable."
        fi
    fi
...
mariadb_start_bg() {
    ...
    ! is_boolean_yes "${NULLATA_DEBUG:-false}" && flags+=("--log-error=/dev/null")
...
# SPDX-License-Identifier: APACHE-2.0
#
# Nullata MySQL Galera library

# shellcheck disable=SC1090,SC1091,SC2119,SC2120

. /opt/nullata/scripts/liblog.sh
. /opt/nullata/scripts/libfs.sh
. /opt/nullata/scripts/libnet.sh
. /opt/nullata/scripts/libos.sh
. /opt/nullata/scripts/libservice.sh
. /opt/nullata/scripts/libvalidations.sh
. /opt/nullata/scripts/libversion.sh
...
!include ${DB_CONF_DIR}/nullata/my_custom.cnf
EOF
}
...
mysql_execute_print_output() {
    ...
    if [[ "${NULLATA_DEBUG:-false}" = true ]]; then
        local mysql_cmd
        mysql_cmd="$(</dev/stdin)"
...
mysql_migrate_old_configuration() {
    ...
    local -r custom_conf_file="$DB_CONF_DIR/nullata/my_custom.cnf"
...
mysql_conf_set() {
    ...
    for section in "${sections[@]}"; do
        if is_boolean_yes "$ignore_inline_comments"; then
            ini_file_set --ignore-inline-comments --section "$section" --key "$key" --value "$value" "$file"
        else
            ini_file_set --section "$section" --key "$key" --value "$value" "$file"
        fi
    done
}
