...
generate_systemd_conf() {
    ...
    local -r service_file="${systemd_units_dir}/nullata.${service_name}.service"
    # Default values
...
[Unit]
Description=Nullata service for ${name}
# Starting/stopping the main nullata service should cause the same effect for this service
PartOf=nullata.service
...
[Install]
# Enabling/disabling the main nullata service should cause the same effect for this service
WantedBy=nullata.service
EOF
}
